...
Sundial is a macOS menu-bar utility that switches the **system-wide** Light/Dark appearance based
...
2. **Never enable App Sandbox.** Sandboxing would block both the SkyLight private framework and
   the location access this app needs. `Sundial.entitlements` intentionally has no
...
5. **Files dropped into `Sundial/` or `SundialTests/` auto-join their target.** The project uses
...
| `just test`  | Runs the hosted `SundialTests` target.                               |
| `just clean` | `xcodebuild clean`.                                                  |
| `just open`  | Opens `Sundial.xcodeproj` in Xcode.                                  |
...
build system. Always build/test via `Sundial.xcodeproj` (through `just` or `xcodebuild`
directly), never `swift build`/`swift test`.

## Tests

`just test` runs the hosted Swift Testing target `SundialTests`. Because tests run inside the
real app process, `SundialApp` checks the `XCTestConfigurationFilePath` environment variable and
...
- Logging goes through the shared `Log` enum (`Sundial/Core/Logger.swift`) — one `Logger` per
